...
public class PhotonConnector : MonoBehaviourPunCallbacks, IOnEventCallback
{
    ...
    public virtual void CreateMatch(MultiplayerAPI.MatchCreationRequest request)
    {
        ...
        var customProperties = new Hashtable() { { "leagueID", PlayerStats.LeagueID } };
        ...
        Debug.Log("<color=cyan>PhotonConnector</color> Creating Match with leagueID: " + PlayerStats.LeagueID);

        RoomOptions roomOptions = new()
        {
            IsOpen = true,
            IsVisible = request.publicMatch,
            MaxPlayers = (byte)request.maxPlayers,
    ...
    public virtual void DestroyMatch()
    {
        ...
        Debug.LogError("<color=red>LeaveRoom DestroyMatch</color>");
        PhotonNetwork.LeaveRoom();
    }

    public virtual void DisconnectFromMatch()
    {
        Debug.LogError("<color=red>LeaveRoom DisconnectFromMatch Match</color>");
        PhotonNetwork.LeaveRoom();
    }
    ...
    {
        Debug.Log($"PhotonConnector.Initialize" );
        ...
        )
        {
            ...
            for (int i = startPage * pageSize; i < this._rooms.Count && i < (startPage + 1) * pageSize; ++i)
            {
                if (this._rooms[i] != null)
                {
                    ...
                    var match = this.GetMatchInformation(this._rooms[i], PlayerStats.LeagueID);
                    if (match != null)
                    {
                        matches.Add(match);
                    }
    ...
    public override void OnCreatedRoom()
    {
        ...
        //// Muneeb: Save player league in room properties
        //Hashtable customProperties = new()
        //{
        //    ["league"] = PlayerStats.LeagueID
        //};
    ...
    public override void OnJoinedRoom()
    {
        ...
        Hashtable customProperties = new()
        {
            ...
            ["profileId"] = ProfileSelector.Instance.CurrentProfile,
    ...
    private void SetPlayersProfiles()
    {
        ...
        {
            if (player != PhotonNetwork.LocalPlayer) // opponent player
            {
                if (player.CustomProperties.TryGetValue("userID", out object userIDObj))
                {
                    FightingGameManager.instance.OpponentID = (int)userIDObj;
                }
                else
                {
                    Debug.LogError("Opponent userID not found");
                }
            }
            if (player.CustomProperties.TryGetValue("Name", out object Name)) {
                if (player.IsMasterClient)
                {
                    FightingGameManager.instance.P1Name = (string)Name;
                }
                else
                {
                    FightingGameManager.instance.P2Name = (string)Name;
                }

            }

                if (player.CustomProperties.TryGetValue("profileId", out object profileIdObj))
            {
                Debug.Log(player.NickName + " has ProfileID: " + (int)profileIdObj);
                ...
                if (PhotonNetwork.LocalPlayer.ActorNumber != player.ActorNumber)
                {
                    ...
                    ProfileSelector.Instance.OpponentProfile = (int)profileIdObj;
                }
                else
                {
                    Debug.Log("My nickName: " + player.NickName);
                    if (player.IsMasterClient)
                    {
                        int profileID = (int)profileIdObj;
                        FightingGameManager.instance.id1 = profileID;
    ...
    #region protected instance methods
    protected virtual MultiplayerAPI.MatchInformation GetMatchInformation(RoomInfo room, int myLeagueID)
    {
        if (debugInfo) Debug.Log("PhotonConnector.GetMatchInformation");
        MultiplayerAPI.MatchInformation match = new()
        {
            //this.averageEloScore =
            currentPlayers = room.PlayerCount,
            isPublic = room.IsOpen && room.IsVisible && !room.RemovedFromList,
            matchName = room.Name,
        ...
        if (room.CustomProperties.TryGetValue("leagueID", out object leagueIDObj))
        {
            if (myLeagueID == (int)leagueIDObj)
            {
                Debug.Log("<color=cyan>PhotonConnector</color> Found match with matching leagueID: " + PlayerStats.LeagueID);
                return match;
            }
            else
            {
                Debug.LogError($"<color=cyan>PhotonConnector</color> LeagueID not matched. myID: {myLeagueID}, theirID: {(int)leagueIDObj}");
                return null;
            }
        }
        else
        {
            Debug.LogError("<color=cyan>PhotonConnector</color> LeagueID not found in room properties");
            return null;
        }
