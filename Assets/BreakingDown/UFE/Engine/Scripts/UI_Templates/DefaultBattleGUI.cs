...
namespace BD
{
	public class DefaultBattleGUI : BattleGUI
	{
			...
			if (msg == UFE.config.selectedLanguage.combo)
			{
				if (this.announcer != null && !this.muteAnnouncer)
				{
					foreach (ComboAnnouncer comboAnnouncer in this.announcer.combos)
					{
						...
						{
							UFE.PlaySound(comboAnnouncer.audio,true);
							break;
						}
					}
				}
		...
		protected virtual string SetStringValues(string msg, ControlsScript controlsScript)
		{
            ...
            if (msg != "FIGHT" && msg != "TIME OVER" && msg!= "K.O." && msg != "PERFECT" && msg!="DRAW")
            {
                msg = TextLocalization.GetLocaliseTextByKey(msg);
            }
		...
		protected override void OnGameBegin(ControlsScript cPlayer1, ControlsScript cPlayer2, StageOptions stage)
		{
			...
			P1 = cPlayer1;
			P2 = cPlayer2;
			FightingGameManager.instance.p1ControlsScript = cPlayer1;
			FightingGameManager.instance.p2ControlsScript = cPlayer2;
			// Mubashir Code
			...
			{
				this.player1GUI.name.text = TextLocalization.GetLocaliseTextByKey( FightingGameManager.instance.P1Name.ToUpper()) ;
			}

			if (this.player2GUI != null && this.player2GUI.name != null)
			{
				this.player2GUI.name.text = TextLocalization.GetLocaliseTextByKey(FightingGameManager.instance.P2Name.ToUpper()); 
			}
		...
		protected override void OnGameEnd(ControlsScript winner, ControlsScript loser)
		{
			...
			this.OnNewAlert(this.SetStringValues("", winner), null); // To remove "extra win screen" as per testers demand.
           
            if (winner == null || loser == null) // round 3 ended in a draw
			{
                // TODO: play draw cutscene
                FightingGameManager.instance.winnerAvatar = FightingGameManager.instance.profiles[ProfileSelector.Instance.CurrentProfile].characterPrefab;
                TimeLineManager.instance.StartOutroCutScene(true);
			}
			else
            {
                FightingGameManager.instance.winnerAvatar = winner.myInfo.characterPrefab;
		...
		protected override async void OnRoundBegin(int roundNumber)
		{
			base.OnRoundBegin(roundNumber);

			if (roundNumber == 1 && player1.controlsScript.myInfo.soundOptions && player2.controlsScript.myInfo.soundOptions) {

				UFE.PlaySound(player1.controlsScript.myInfo.openingSound,true);
				await Task.Delay((int) player1.controlsScript.myInfo.openingSound.length * 1000);
                await Task.Delay(500);
                UFE.PlaySound(player2.controlsScript.myInfo.openingSound,true);
                await Task.Delay((int)player2.controlsScript.myInfo.openingSound.length * 1000);
				await Task.Delay(500);
            }
			if(roundNumber ==1)
            UFE.PlayMusic(UFE.GetStage().music);
            if (this.player1GUI != null && this.player1GUI.alert != null && this.player1GUI.alert.text != null)
			{
				this.player1GUI.alert.text.text = string.Empty;
			}
			...
			else if (roundNumber < UFE.config.roundOptions.totalRounds)
			{
				...
				if (this.announcer != null && !this.muteAnnouncer)
				{
					if (roundNumber == 1) UFE.PlaySound(this.announcer.round1,true);
					if (roundNumber == 2) UFE.PlaySound(this.announcer.round2, true);
					if (roundNumber == 3) UFE.PlaySound(this.announcer.round3, true);
					if (roundNumber > 3) UFE.PlaySound(this.announcer.otherRounds, true);
				}
			...
			else
			{
				...
				if (this.announcer != null && !this.muteAnnouncer)
				{
					UFE.PlaySound(this.announcer.finalRound, true);
				}
            ...
            UFE.DelaySynchronizedAction(UFE.StartFight, 32);
        }
		...
		protected override void OnRoundEnd(ControlsScript winner, ControlsScript loser)
		{
...

			{
				...
				// Muneeb
				// To show player loses text
				//if (winnerPlayer == 1)
				//{
				//    //Debug.LogError("Player Victory Text");
				//    this.OnNewAlert(this.SetStringValues(UFE.config.selectedLanguage.victory, winner), null);
				//}
				//else if (winnerPlayer == 2)
				//{
				//	this.OnNewAlert(this.SetStringValues(UFE.config.selectedLanguage.loss, loser), null);
				//}
				// End
				UFE.stopBGM();
		...
		{
			base.OnTimeOver();
			this.OnNewAlert("TIME OVER",null);
