...
public class FluxCapacitor {
    ...
    protected async void CheckEndRoundConditions() {
        if (UFE.GetControlsScript(1).currentLifePoints == 0 || UFE.GetControlsScript(2).currentLifePoints == 0) {

			UFE.pauseMusic();
			if (UFE.GetControlsScript(1).currentLifePoints == 0) { if (UFE.GetControlsScript(1).myInfo.soundOptions) {  UFE.PlaySound(UFE.GetControlsScript(1).myInfo.koSound,true); } else UFE.PlaySound(UFE.GetControlsScript(1).myInfo.deathSound); }
			if (UFE.GetControlsScript(2).currentLifePoints == 0) { if (UFE.GetControlsScript(2).myInfo.soundOptions) { UFE.PlaySound(UFE.GetControlsScript(2).myInfo.koSound,true); } else UFE.PlaySound(UFE.GetControlsScript(2).myInfo.deathSound); }
            ...
            if (UFE.config.roundOptions.slowMotionKO) {
                ...
                UFE.GetControlsScript(1).shakeCamera = false;
                UFE.GetControlsScript(2).shakeCamera = false;
			
                UFE.UFEInstance.StartCoroutine(PauseGameForOneSecond());
            } else {
    ...
    IEnumerator PauseGameForOneSecond()
    {var pretimespan = UFE.timeScale;
        // Pause game time
        UFE.timeScale = 0.01;
        UFE.fixedDeltaTime = 0.02f * (float)UFE.timeScale; // adjust physics update rate
		bool ko = false;
        float elapsed = 0f;

        while (elapsed < UFE.config.roundOptions._slowMoTimer)
        {
            elapsed += Time.unscaledDeltaTime; // real time, not affected by timeScale
            UFE.timeScale = Mathf.Lerp(0.01f, 1.2f, elapsed / (float)UFE.config.roundOptions._slowMoTimer);
            UFE.fixedDeltaTime = 0.02f * (float)UFE.timeScale; // update physics step
            yield return null;
			if(elapsed> UFE.config.roundOptions._slowMoTimer / 2 && !ko) { ko = true; UFE.FireAlert(UFE.config.selectedLanguage.ko, null); }
        }

        // Ensure time is back to normal
        UFE.timeScale = 1.2f;
        UFE.fixedDeltaTime = 0.02f;
        // Wait for 1 real second (ignores timeScale)
        /*
                UFE.timeScale =  UFE.config.roundOptions._slowMoSpeed;
                // Resume game time
                UFE.FireAlert(UFE.config.selectedLanguage.ko, null);
                UFE.DelaySynchronizedAction(ReturnTimeScale, UFE.config.roundOptions._slowMoTimer);*/

       
        UFE.DelaySynchronizedAction(EndRound, 1 / UFE.config.roundOptions._slowMoTimer);
    ...
    public void EndRound() {
		...
		if (UFE.timer == 0)
		{
			...
			if (p1LifePercentage == p2LifePercentage)
			{
				...
				{
                    // draw scenario
                    // if both players have the same amount of rounds won, the game will end in a draw. // Muneeb
					if (p1ControlsScript.roundsWon == p2ControlsScript.roundsWon)
					{
                        UFE.DelaySynchronizedAction(this.EndMatch, UFE.config.roundOptions._endGameDelay);
                        UFE.FireGameEnds(null, null);
                        return;
                    }

                    // winner is the player with more rounds won. // Muneeb
                    winner = (p1ControlsScript.roundsWon > p2ControlsScript.roundsWon) ? p1ControlsScript : p2ControlsScript;
					loser = (winner == p1ControlsScript) ? p2ControlsScript : p1ControlsScript;

                    UFE.DelaySynchronizedAction(this.EndMatch, UFE.config.roundOptions._endGameDelay);
                    UFE.FireGameEnds(winner, loser);
					return;
                }
			}
			else
    ...
    protected virtual void ExecuteLocalDelayedActions() {
        ...
        for (int i = UFE.delayedLocalActions.Count - 1; i >= 0; --i) {
            ...
            if (action.steps <= 0) {
                action.action();
				if (UFE.delayedLocalActions.Count > 0) Debug.Log("StuckHere......" + i + "   " + UFE.delayedLocalActions.Count);
