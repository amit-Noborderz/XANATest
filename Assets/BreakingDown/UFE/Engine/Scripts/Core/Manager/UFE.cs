...
public class UFE : MonoBehaviour, UFEInterface
{
    ...
    public static GlobalInfo config { get { return GlobalInfo; } set { GlobalInfo = value; Debug.LogError("global Config Update "+GlobalInfo.name + " Scene Path " + GlobalInfo.stages[0].stagePath); } }
	...
	protected static readonly string BgmVolumeKey = "BgmVolume";
	...
	private static AudioSource BGM_Audio;
	private static AudioSource soundsAudioSource;
    public static float BGM_Percent, Music_Percent, SoundFX, Dialog;
	...
	public static float GetMusicVolume(){
		if (UFE.config != null) return config.DefalutMusicVolume;
		return 1f;
	}

    public static float GetBGMVolume()
    {
        if (UFE.config != null) return config.DefalutBgmVolume;
        return 0.1f;
    }

    public static float GetSoundFXVolume(){
		if (UFE.config != null) return UFE.config.DefalutSoundfxVolume;
		return 1f;
	}

	public static void InitializeAudioSystem(){
        //Camera cam = Camera.main;
        Camera cam = GameObject.Find("BreakingDownGameCamera").GetComponent<Camera>();
		// Create the AudioSources required for the music and sound effects
		UFE.BGM_Audio = cam.GetComponent<AudioSource>();
		...
		UFE.musicAudioSource.volume = config.DefalutMusicVolume;
		...
		UFE.soundsAudioSource.volume = config.DefalutSoundfxVolume;
	}
    ...
    {
        if (UFE.BGM_Audio.clip != null) return UFE.BGM_Audio.isPlaying;
        return false;
    }
	...
	{
		UFE.musicAudioSource.Pause();
	}
	public static void stopBGM()
	{
		UFE.BGM_Audio.Stop();
		UFE.BGM_Audio.volume = 1f;
	}
	public static void playBGM(AudioClip music)
	{
		UFE.BGM_Audio.clip = music;
        UFE.BGM_Audio.volume = UFE.config.DefalutBgmVolume * UFE.BGM_Percent;
        UFE.BGM_Audio.Play();	
	...
	public static void PlayMusic(AudioClip music){
		if (music != null){
			AudioClip oldMusic = UFE.GetMusicClip();
					UFE.musicAudioSource.volume = UFE.config.DefalutMusicVolume* UFE.Music_Percent;
			if (music != oldMusic){
    ...
    public static void PlayBGM(AudioClip music)
    {
        if (music != null)
        {
            AudioClip oldMusic = UFE.GetBGMClip();
            ...
            if (config.music && (music != oldMusic || !UFE.IsPlayingBGM()))
            {
                UFE.BGM_Audio.Play();
            }
    ...
    public static void PlaySound(IList<AudioClip> sounds, bool slowDown = false)
    {
		if (sounds.Count > 0){
			UFE.PlaySound(sounds[UnityEngine.Random.Range(0, sounds.Count)],slowDown);
		}
	}
	
	public static async void PlaySound(AudioClip soundFX,bool slowDown = false){
		var volume = UFE.musicAudioSource.volume;

        if (slowDown)
		{
			UFE.musicAudioSource.volume = UFE.musicAudioSource.volume*.5f;

			Debug.Log($"play sound  {soundFX.name}  volume  {UFE.Dialog * UFE.config.DefalutSoundfxVolume}");
			UFE.PlaySound(soundFX,UFE.Dialog*UFE.config.DefalutSoundfxVolume);
			await Task.Delay( (int)Mathf.Ceil( soundFX.length * 1000));
			UFE.musicAudioSource.volume = volume;
		}else
            UFE.PlaySound(soundFX, UFE.SoundFX * UFE.config.DefalutSoundfxVolume);
    }
	...
	public static void SetMusicVolume(float volume){
		if (UFE.config != null) UFE.config.DefalutMusicVolume = volume;
		if (UFE.musicAudioSource != null) UFE.musicAudioSource.volume = volume*UFE.Music_Percent;
    ...
    {
        if (UFE.config != null) UFE.config.DefalutBgmVolume = volume;
        if (UFE.BGM_Audio != null) UFE.BGM_Audio.volume = volume*UFE.BGM_Percent;

        PlayerPrefs.SetFloat(UFE.BgmVolumeKey, volume);
        PlayerPrefs.Save();
    }
    ...
    public static void SetSoundFXVolume(float volume){
		if (UFE.config != null) UFE.config.DefalutSoundfxVolume = volume;
        if (UFE.soundsAudioSource != null) UFE.soundsAudioSource.volume = volume ;
	...
	public static void StartMainMenuScreen(float fadeTime){
        ...
        if (UFE.stageInstance) { DestroyImmediate(stageInstance); }
	}
	...
	public static int localPlayerNum = -1;
	...
	{
		if (winner == null && loser == null && UFE.OnGameEnds != null) // if round 3 ended in a draw
		{
			UFE.player1WonLastBattle = false;
			UFE.OnGameEnds(null, null);
        }
		else
		{
	...
	/*public  void invoke() { Invoke(nameof(SlowKO), .1f); }*/
    public void SlowKO()
    {
        UFE.timeScale = UFE.timeScale * UFE.config.roundOptions._slowMoSpeed;
        UFE.DelaySynchronizedAction(fluxCapacitor.ReturnTimeScale, UFE.config.roundOptions._slowMoTimer);

        UFE.DelaySynchronizedAction(fluxCapacitor.EndRound, 1 / UFE.config.roundOptions._slowMoSpeed);
    }
	...
	public static void setSOundDefault()
	{
		...
		UFE.config.bgm = true;
        UFE.SetBGMVolume(PlayerPrefs.GetFloat(UFE.BgmVolumeKey, 1f));
	...
	protected static void OnDisconnectedFromServer() {
		...
		if (!UFE.closing){
			...
			if (UFE.config.lockInputs && UFE.currentScreen == null){
				...
				Debug.LogError("Connection lost screen OnDisconnectedFromServer");
				UFE.StartConnectionLostScreen();
            }
		}
	}
    ...
    public static Text DebuggerText(string dName, string dText, Vector2 position, TextAnchor alignment)
    {
      ...
      //  Font ArialFont = (Font)Resources.GetBuiltinResource(typeof(Font), "LegacyRuntime.ttf"); // Arial.ttf
	...
	private static void _StartConnectionLostScreen(float fadeTime){
		UFE.EnsureNetworkDisconnection();
		Debug.Log("Oppen ConnectionLost");
		UFE.HideScreen(UFE.currentScreen);
		if (UFE.config.gameGUI.connectionLostScreen == null){
            ...
            UFE.ShowScreen(UFE.config.gameGUI.connectionLostScreen);  Debug.LogError("Show Connection LostScreen");
	...
	static GameObject stageInstance;
    private static void _StartGame(float fadeTime){
		...
		{
			Debug.Log("Stage Sewtup..." + config.selectedStage.prefab.name);
			if (UFE.config.selectedStage.prefab != null)
			{
				stageInstance = Instantiate(config.selectedStage.prefab);
                Scene targetScene = SceneManager.GetSceneByName("BreakingDownGame");
                SceneManager.MoveGameObjectToScene(stageInstance, targetScene);
                //stageInstance.transform.parent = gameEngine.transform;
                //config.selectedStage.prefab = null;
            }
			else
			{
		...
		else
        {
            ...
            Debug.Log("Selected Stage " + UFE.config.selectedStage.stageName + $"   configname == {UFE.config.name}");
	...
	public static void _StartGameGUI(float fadeTime)
    {
        ...
        Scene targetScene = SceneManager.GetSceneByName("BreakingDownGame");
        SceneManager.MoveGameObjectToScene(UFE.gameEngine, targetScene);
    ...
    public static void FindAndSpawnAssist(ControlsScript controlsScript, int player) {
       ...
       //                     ChangeStats(move, 300f,50f);//instead of 300f put value taken from sheet and replace 50f with opponent defense value
       //                     break;
       //                 case "Light Punch":
							//ChangeStats(move, 300f, 50f);//instead of 300f put value taken from sheet and replace 50f with opponent defense value
							//break;
