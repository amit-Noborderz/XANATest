        ...
        public void OnGUI()
        {
            ...
            scrollPos = EditorGUILayout.BeginScrollView(scrollPos);
            {
                ...
                EditorGUILayout.BeginVertical(rootGroupStyle);
                {
                    ...
                    if (prefabsOption)
                    {
                        ...
                        if (characterInfo.characterPrefabStorage == StorageMode.Prefab) {
                            ...
                            characterInfo.ClothingAsset = (GameObject)EditorGUILayout.ObjectField("Cloth:", characterInfo.ClothingAsset, typeof(UnityEngine.GameObject), true);
                        } else {
                            ...
                            ValidatePrefab(characterInfo.ClothingAsset, false);
                            if (characterWarning)
                            {
                                GUILayout.BeginHorizontal("GroupBox");
                ...
                EditorGUILayout.BeginVertical(rootGroupStyle);
                {
                    EditorGUILayout.BeginHorizontal();
                    {
                       soundOptions = EditorGUILayout.Foldout(soundOptions, "Sound Effects", foldStyle);
                        helpButton("character:soundeffects"); // Optional help link
                    }
                    EditorGUILayout.EndHorizontal();
                    ...
                    if (soundOptions)
                    {
                        EditorGUILayout.BeginVertical(subGroupStyle);
                        {
                            EditorGUILayout.Space();
                            EditorGUI.indentLevel += 1;
                            characterInfo.soundOptions = EditorGUILayout.Toggle("Override Default Sound", characterInfo.soundOptions);
                            characterInfo.openingSound = (AudioClip)EditorGUILayout.ObjectField("Opening Sound:", characterInfo.openingSound, typeof(AudioClip), false);
                            characterInfo.koSound = (AudioClip)EditorGUILayout.ObjectField("K.O. Sound:", characterInfo.koSound, typeof(AudioClip), false);
