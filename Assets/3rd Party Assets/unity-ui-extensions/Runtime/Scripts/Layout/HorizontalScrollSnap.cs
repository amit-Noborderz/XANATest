...
namespace UnityEngine.UI.Extensions
{
    ...
    public class HorizontalScrollSnap : ScrollSnapBase, IEndDragHandler
    {
        ...
        void Start()
        {
            ...
            if (isBD)
            {
                _currentPage = ProfileSelector.Instance.CurrentProfile;
                indexToStart = ProfileSelector.Instance.CurrentProfile;
                if (ProfileSelector.Instance.CurrentProfile > PlayerPrefs.GetInt("OpponentID", -1))
                {
                    _currentPage--;
                    indexToStart--;
                }
        ...
        {
            _scrollStartPosition = _screensContainer.anchoredPosition.x;
            _scroll_rect.horizontalNormalizedPosition = (float)(indexToStart) / (_screens - 1);
            OnCurrentScreenChange(indexToStart);
        }
