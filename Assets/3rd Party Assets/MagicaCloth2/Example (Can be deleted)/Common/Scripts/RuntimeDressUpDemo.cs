...
namespace MagicaCloth2
{
    ...
    public class RuntimeDressUpDemo : MonoBehaviour
    {
        ...
        int hairIndex = 0;
        int costumeIndex = 0;
        public void OnHairEquipButtonList()
        {
            if (hairEquipInfo.IsValid())
                Remove(hairEquipInfo);

            Equip(hairList[hairIndex], hairEquipInfo);

            hairIndex++;
            if (hairIndex >= hairList.Count)
                hairIndex = 0;
        }
        public void OnBodyEquipButtonList()
        {
            if (bodyEquipInfo.IsValid())
                Remove(bodyEquipInfo);

            Equip(costumeList[costumeIndex], bodyEquipInfo);

            costumeIndex++;
            if (costumeIndex >= costumeList.Count)
                costumeIndex = 0;
        }
